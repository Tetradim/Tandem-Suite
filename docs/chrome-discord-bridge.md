...
Sentinel Core accepts local Chrome bridge traffic at:
...
`message` payloads are normalized and appended to the Sentinel Core local event bus as `signal.observed` with `contract_version: chrome.discord.message.v1`.
